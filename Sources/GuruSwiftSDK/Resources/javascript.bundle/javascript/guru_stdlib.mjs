import {
   ...
   */
  export const Keypoint = GURU_KEYPOINTS.reduce((keypointEnum, keypointName, index) => {
  ...
  export class Frame {
    ...
    async findObjects(objectTypes, {attributes = {}, keypoints = true} = {}) {
      const objectBoundaries = await this._findObjectBoundaries(objectTypes, attributes);
  
      return await Promise.all(objectBoundaries.map(async (nextObject) => {
        let objectKeypoints = null;
        if (keypoints && nextObject.type === "person") {
          objectKeypoints = await this._findObjectKeypoints(nextObject.boundary);
        }
    ...
    async _findObjectBoundaries(objectTypes, attributes) {
      if (objectTypes === "person" || objectTypes === "people") {
        ...
        const preprocessedFrame = preprocessImageForObjectDetection(this.image, 768, 768);
        const imageTensor = preprocessedImageToTensor(this.guruModels.ort(), preprocessedFrame);
  
        let objectBoundaries = [];
        const textBatches = prepareTextsForOwlVit(objectTypes);
        for (const textBatch of textBatches) {
          const textTensors = this.guruModels.tokenizer().tokenize(
            textBatch,
            { padding: true, max_length: 16 }
          );
          ...
          objectBoundaries = objectBoundaries.concat(
            postProcessObjectDetectionResults(results, textBatch)
              .map((nextObject) => {
                return {
                  "type": nextObject.class,
                  "boundary": new Box(
                    new Position(nextObject.bbox[0], nextObject.bbox[1], nextObject.probability),
                    new Position(nextObject.bbox[2], nextObject.bbox[3], nextObject.probability),
                  )
                }})
          );
        }
    ...
    async _findPeopleBoundaries() {
      ...
      const preprocessedFrame = preprocessImageForPersonDetection(this.image, 416, 416);
      const tensor = preprocessedImageToTensor(this.guruModels.ort(), preprocessedFrame);
    ...
    async _findObjectKeypoints(boundingBox) {
      // TODO: use the bounding-box
      const { keypoints } = onnx.runInference(this.image);
      const jointToLocation = {};
      GURU_KEYPOINTS.forEach((keypointName, index) => {
        if (index >= 17) {
          return;
        }
        const {x, y, score} = keypoints[index];
        jointToLocation[keypointName] = new Position(x, y, score);
      });
